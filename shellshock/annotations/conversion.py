...
from shellshock.perception.world import DetectionBox, PoseKeypoint, World, build_world_from_image, build_world_from_image_with_diagnostics
...
def load_manual_scene(
    ...
    # Full physical circles take precedence over clipped training rectangles.
    circle_rows=geometry.get('circles',[])
    if circle_rows:
        names={CLASS_NAMES[int(item['class_id'])] for item in circle_rows}
        boxes=[box for box in boxes if box.name not in names]
        for item in circle_rows:
            x,y=map(float,item['center']);radius=float(item['radius'])
            if radius>0:
                boxes.append(AnnotationBox(CLASS_NAMES[int(item['class_id'])],x-radius,y-radius,2*radius,2*radius,source='manual'))
    lines = []
...
def annotations_to_world_with_diagnostics(scene: SceneAnnotation, image) -> tuple[World, tuple[float, float] | None, list[dict[str, object]]]:
    detections = [DetectionBox(item.name, item.x, item.y, item.width, item.height, item.confidence, item.keypoints, item.source) for item in scene.boxes]
    ...
    if scene.lines:
        from dataclasses import replace
        from shellshock.perception.world import LineObstacle
...
def save_manual_scene(scene: SceneAnnotation, label_path: Path, geometry_path: Path, metadata_path: Path | None = None, *, backup=True) -> None:
    ...
    circular={'obstacle_circle','portal_orange','portal_blue','blackhole','double_damage','Triple_damage'}
    circles=[{'class_id':next(k for k,v in CLASS_NAMES.items() if v==item.name),
              'center':list(item.center),'radius':(item.width+item.height)/4}
             for item in scene.boxes if item.name in circular]
    geometry_path.write_text(json.dumps({"circles":circles,"lines": [{"start": list(item.start), "end": list(item.end)} for item in scene.lines], "self_center": list(scene.self_center) if scene.self_center else None, "self_muzzle":list(scene.self_muzzle) if scene.self_muzzle else None,"deleted": scene.deleted}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    if metadata_path is not None:
        metadata_path.parent.mkdir(parents=True, exist_ok=True)
        metadata_path.write_text(json.dumps(scene.metadata, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def scene_from_editor(circles,lines,width,height,center,muzzle=None,metadata=None):
    boxes=[AnnotationBox(CLASS_NAMES[c.class_id],c.center_x-c.radius,c.center_y-c.radius,2*c.radius,2*c.radius,source='manual') for c in circles]
    for line in lines:
        x0=min(line.start[0],line.end[0])-3;y0=min(line.start[1],line.end[1])-3
        boxes.append(AnnotationBox('obstacle_line',x0,y0,abs(line.end[0]-line.start[0])+6,abs(line.end[1]-line.start[1])+6,source='manual'))
    if center is not None:
        # Keep the existing class-1 shortcut row for dataset compatibility;
        # self_center in geometry is the authoritative semantic point.
        boxes.append(AnnotationBox(CLASS_NAMES[1],center[0]-8,center[1]-8,16,16,source='manual'))
    return SceneAnnotation(width,height,boxes,[LineAnnotation(tuple(line.start),tuple(line.end)) for line in lines],center,muzzle,dict(metadata or {}))
