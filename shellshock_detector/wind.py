...
from .digit_recognizer import recognize_trained_digits
...
def _number_from_panel(panel: np.ndarray) -> int | None:
    model_value, confidence = recognize_trained_digits(panel)
    if model_value is not None and confidence >= TRAINED_DIGIT_CONFIDENCE_THRESHOLD:
        try:
